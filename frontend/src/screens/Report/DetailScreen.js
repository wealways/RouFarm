...
const Detail = ({ route }) => {
  ...
  useEffect(() => {
    instance
      .get(`report/dailyAPI/${year}${month}${day}`, {
        headers: {
    ...
    <LinearGradient
      ...
      <Contents>
        ...
        <View style={styles.result}>
          {rate == 100 && <Text style={styles.comment}>훌륭합니다!! 👍👍</Text>}
          {rate >= 50 && rate < 100 && (
            <Text style={styles.comment}>잘하고 있어요. 조금만 더 노력합시다! 🤜</Text>
          )}
          {rate < 50 && <Text style={styles.comment}>열심히 하셔야 합니다 👿</Text>}
          <View style={styles.rate}>
            <Text style={{ fontSize: 20, borderBottomWidth: 1, paddingRight: 10 }}>달성 :</Text>
            <Text style={{ fontSize: 30, borderBottomWidth: 1 }}>{rate.toFixed(1)}%</Text>
          </View>
        </View>
      </Contents>
    </LinearGradient>
